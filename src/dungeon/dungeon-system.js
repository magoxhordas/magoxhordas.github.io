...
const DNG={
  ...
  /* ── Efeitos elementais ──
     A dungeon nao tinha nenhuma nocao de elemento: os projeteis inimigos ja'
     carregavam _fire, _poison e _freeze, e o pantano ja' envenenava, mas nada
     disso virava imagem. Estas tres funcoes trazem para ca' o mesmo
     vocabulario da campanha (corpo pega a cor do elemento + faiscas), so' que
     pelo caminho de PALETA, que e' como a dungeon desenha — o mesmo truque que
     ela ja' usa no estado sombra e no clarao de dano. */
  ...
  _paletaEl(pal,cor,forca){
    // mistura a paleta com a cor do elemento, em vez de trocar tudo: assim o
    // sprite continua legivel por baixo da cor
    ...
    for(const k in pal){
      ...
      const o=[parseInt(v.slice(1,3),16),parseInt(v.slice(3,5),16),parseInt(v.slice(5,7),16)];
      out[k]='#'+[0,1,2].map(i=>Math.round(o[i]+(alvo[i]-o[i])*forca).toString(16).padStart(2,'0')).join('');
    }
    return out;
  },
  _faiscasEl(ctx,el,sx,sy,raio,ts){
    ...
    if(!cor)return;
    const sobe=(el==='ice')?-0.5:1.3, vel=(el==='solar'||el==='fire')?0.0009:0.0005;
    ctx.save(); ctx.fillStyle=cor;
    for(let i=0;i<3;i++){
      const p=((ts*vel+i/3+sx*0.01)%1);
      ctx.globalAlpha=(1-p)*0.8;
      ctx.fillRect(Math.round(sx+Math.sin(sx*0.1+i*2.6+p*3.1)*raio*0.42),
                   Math.round(sy-p*raio*sobe), 2,2);
    }
    ctx.restore();
  },
  ...
  _update(dt,ts){
    ...
    for(let i=this.projectiles.length-1;i>=0;i--){
      ...
      if(p._rangedWpn){
        ...
        // A MARCA sai do sorteio: o dano extra continua sendo 50%, mas quem
        // levou cuspe de veneno tem de PARECER envenenado sempre. Com a marca
        // dentro do if, o efeito visual so' aparecia em metade dos acertos.
        if(p._poison){
          this._marcarEl(this,'poison',1800);
          if(Math.random()<0.5){this._takeDmg(p.dmg*0.4);this._ft(this.px,this.py-36,'☠️ VENENO!','#44ff44');}
        }
    ...
    if(this._biome===BIOMES.ice){
      for(const e of this.entities){
        ...
        // enquanto o bioma gelado os segura lentos, eles ficam azulados —
        // e' a mesma leitura da campanha: "lento" mostra a cor do gelo
  ...
  _render(ts){
    ...
    [...this.entities].filter(e=>!e.dead||e._shadowBound).sort((a,b)=>a.y-b.y).forEach(e=>{
      ...
      drawSpriteAt(sprArr, elE?this._paletaEl(e.palFn(),this._CORES_EL[elE]||'#ffffff',0.55):e.palFn(),
                   esx,esy+e.r,e.dir==='right',e.sc);
    ...
    if(!(this.pInvTimer>0&&Math.floor(this.pInvTimer/80)%2===0)){
      ...
      if(!drawHeroVisual(heroVisual,psx,psy+14,flipX)){
        drawSpriteAt(heroVisual.sprite,
          elP?this._paletaEl(heroVisual.pal,this._CORES_EL[elP]||'#ffffff',0.55):heroVisual.pal,
          psx,psy+14,flipX,1);
      }else if(elP&&typeof tingirHeroi==='function'){
        // o heroi da dungeon usa a MESMA arte da campanha, entao a tintura
        // de la' serve aqui sem duplicar codigo
        const w=heroVisual.imgFrame||(typeof HERO_IMG_FRAME!=='undefined'?HERO_IMG_FRAME:48);
        tingirHeroi(ctx,{img:heroVisual.img,w,h:w,flip:flipX,
          dx:Math.round(psx-w/2),
          dy:Math.round((psy+14)-(heroVisual.imgFeet||(typeof HERO_IMG_FEET!=='undefined'?HERO_IMG_FEET:42)))},
          this._CORES_EL[elP]||'#ffffff', 0.42);
      }
