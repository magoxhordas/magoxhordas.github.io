...
const chapterData=readFileSync(path.join(root,'src','campaign','chapter-data.js'),'utf8');
const campaignSystem=readFileSync(path.join(root,'src','campaign','campaign-system.js'),'utf8');
const campaignSource=[html,chapterData,campaignSystem].join('\n');
...
assert.match(campaignSource,/const CAMPAIGN_CHAPTER_DURATION=6000;/,'A abertura deve durar exatamente 6 segundos.');
...
assert.match(campaignSource,/state==='chapter'/,'O estado de capítulo deve pausar o loop da campanha.');
...
assert.match(campaignSource,/finishCampaignChapter[\s\S]*Audio\.playCombatMusic\(chapter\.arena\)/,'A música do bioma deve começar após a abertura.');
...
for(const chapter of chapters){
  ...
  assert.match(campaignSource,configPattern,`Configuração ausente para a onda ${chapter.wave}.`);
}
...
for(const effect of ['ash','green-spark','dark-dust','stone','web-thread','spore','red-dust','eye','snow','crystal','ice-fragment','sand','gold-dust','rolling-stone','ember','black-ash','orange-spark','smoke']){
  assert.ok(campaignSource.includes(`'${effect}'`),`Efeito temático ausente: ${effect}`);
}
