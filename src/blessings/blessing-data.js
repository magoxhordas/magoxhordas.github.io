...
const A=(id,name,icon,desc,role='ASCENSÃO',extra={})=>({id,name,icon,desc,role,isAscension:true,standalone:true,...extra});
const DEITIES=[
...
B('zeus_storm_mark','Marca da Tempestade','✦','critMark',V(.40,.55,.75,1.00),'Concede uma pequena chance de crítico. Críticos marcam o alvo; o próximo golpe consome a marca em uma descarga brutal.','CRÍTICO',{critChance:V(.02,.03,.04,.055),radius:[60,70,80,95,110]}),
...
B('ares_rampage','Frenesi de Guerra','🔥','rampage',V(.10,.14,.18,.24),'Eliminações acumulam Fúria. No limite, você entra em Frenesi e precisa continuar matando para mantê-lo.','COMBO',{stacks:[5,5,5,4,4],duration:[5000,5200,5500,6000,6500],attack:V(.10,.14,.20,.28)}),
B('ares_execution','Sem Misericórdia','🗡','execution',V(.18,.24,.32,.42),'O golpe que cruza o limiar executa inimigos comuns. Elites e chefes recebem dano adicional quando estão feridos.','EXECUÇÃO',{threshold:[.12,.14,.16,.18,.20],eliteBonus:V(.28,.38,.52,.70),bossBonus:V(.10,.14,.20,.28)}),
...
B('hecate_hex','Selo da Encruzilhada','☾','hex',V(.55,.75,1.00,1.35),'Alternar entre ataque básico, arsenal, golpe após dash ou invocação detona a maldição do alvo.','MAGIA',{marks:[2,2,2,1,1],radius:[70,80,90,105,120]}),
...
B('hecate_triple_path','Caminhos Cruzados','△','triple',V(.18,.24,.32,.42),'Combinar um golpe após dash ou uma invocação com outra fonte desperta Tríplice Poder; uma terceira fonte prolonga o efeito.','COMBO',{sources:[2,2,2,2,2],duration:[3500,4000,4500,5000,5500]}),
B('hecate_witchfire','Fogo das Bruxas','✧','critSplash',V(.35,.50,.70,.95),'Concede uma pequena chance de crítico. Críticos abrem uma chama violeta que atinge inimigos ao redor do alvo.','CRÍTICO',{critChance:V(.02,.03,.04,.055),radius:[85,95,105,120,135],targets:[2,2,3,3,4]}),
...
B('selene_moon_harvest','Colheita da Lua','🌙','moonHarvest',V(.012,.017,.024,.032),'Grupos de abates curam e aceleram o próximo Raio de Luar. Abates durante Lua Cheia contam em dobro.','SUSTENTAÇÃO',{kills:[3,3,3,2,2],cooldownCut:[700,850,1000,1200,1450]})
]),
...
B('atena_perfect_stance','Postura Perfeita','♜','stance',V(.025,.035,.05,.07),'Ficar ileso acumula Postura. As cargas aumentam defesa e dano; sofrer dano remove parte delas.','TÉCNICA',{max:[4,4,5,5,5],damage:V(.01,.015,.02,.03)}),
B('atena_tactical_mark','Brecha Tática','⌖','tactical',V(.55,.75,1.00,1.35),'Acertos repetidos revelam uma brecha. O primeiro ataque após dash consome a marca em um golpe calculado.','TÉCNICA',{hits:[6,6,5,5,4]}),
...
B('dionisio_double_toast','Brinde Duplo','🥂','buffDuration',V(.20,.30,.45,.65),'No início de cada onda, concede um bônus aleatório. Todos os bônus temporários de Dionísio duram mais e podem se sobrepor.','CAOS',{toastPower:V(.05,.07,.10,.14),toastDuration:[3500,3800,4200,4700,5300]}),
B('dionisio_blackout','Apagão','♬','buffBurst',V(.55,.75,1.00,1.35),'Três efeitos temporários armam uma explosão no próximo abate. Sem bônus, uma sequência de eliminações também apaga as luzes.','CAOS',{needed:[3,3,3,3,2],fallbackKills:[9,8,7,6,5],radius:[100,115,130,150,170]}),
...
B('hefesto_shrapnel','Estilhaços da Forja','✦','shrapnel',V(.35,.50,.70,.95),'Concede uma pequena chance de crítico. Abates críticos lançam estilhaços contra inimigos próximos.','CRÍTICO',{critChance:V(.02,.03,.04,.055),targets:[2,2,3,3,4],radius:[150,160,175,190,210]}),
...
B('artemis_ricochet','Flecha Ricochete','➶','critRicochet',V(.40,.55,.75,1.00),'Concede uma pequena chance de crítico. Críticos ricocheteiam para outro inimigo próximo.','CRÍTICO',{critChance:V(.02,.03,.04,.055),targets:[1,1,1,2,2],radius:[180,195,210,230,250]}),
...
B('artemis_volley','Chuva de Flechas','✧','critVolley',V(.35,.50,.70,.95),'Concede uma pequena chance de crítico. Depois de alguns críticos, dispara automaticamente uma saraivada contra outros inimigos.','CRÍTICO',{critChance:V(.02,.03,.04,.055),crits:[4,4,3,3,2],targets:[2,2,3,3,4],radius:[230,245,260,280,310]})
]),
...
B('poseidon_tide','Maré Crescente','↑','tide',V(.12,.17,.23,.30),'Acertos em sequência fazem a Maré subir. Manter o ritmo aumenta o dano até o limite.','COMBO',{gain:[.010,.012,.014,.017,.020],decay:[2200,2400,2600,2900,3300]}),
B('poseidon_crush','Pressão Abissal','◉','crush',V(.55,.75,1.00,1.35),'Acertos acumulam Pressão. Empurrões das ondas de Poseidon contam novamente e aceleram o esmagamento.','MARÉ',{hits:[4,4,4,3,3]}),
...
B('sauron_ring','O Anel','◉','ring',V(.12,.16,.22,.30),'Amplifica dano, defesa, movimento e crescimento das outras bênçãos, mas cobra vida máxima imediatamente. A ordem das escolhas não altera o preço.','RISCO',{hpPenalty:V(.04,.055,.075,.10)}),
...
B('nazgul_phantom_blade','Lâmina Fantasma','✦','phantom',V(.035,.05,.07,.10),'Concede uma pequena chance de crítico. Críticos abrem uma janela de esquiva e reduzem a recarga do dash.','CRÍTICO',{critChance:V(.02,.03,.04,.055),duration:[1800,2000,2200,2500,3000],dashCut:[120,150,190,240,320]}),
...
const ASCENSIONS={
zeus:[
A('zeus_asc_storm_avatar','Avatar da Tempestade','⚡','A cada seis acertos, uma descarga própria salta entre inimigos. Bênçãos de tempestade ganham salto, área, dano e preservam parte da carga.','ASCENSÃO · TEMPESTADE',{hits:6,targets:2,mult:.35,radius:170,synergies:['zeus_chain_lightning','zeus_static_charge','zeus_thunder_dash','zeus_storm_mark','zeus_last_storm']}),
...
ares:[
A('ares_asc_god_of_war','Deus da Guerra','⚔','Quatro eliminações despertam um Frenesi próprio. Frenesi de Guerra exige menos Fúria, dura mais e Retaliação recebe uma carga extra.','ASCENSÃO · FRENESI',{kills:4,duration:5000,damage:.12,attack:.12,synergies:['ares_rampage','ares_retaliation']}),
A('ares_asc_blood_banquet','Banquete de Sangue','🩸','Cinco acertos abrem uma ferida explosiva e recuperam vida. Marca de Sangue e Condenação também espalham feridas ao detonar.','ASCENSÃO · SANGRAMENTO',{hits:5,mult:.50,radius:90,heal:.01,synergies:['ares_blood_mark','ares_doom']})
],
hecate:[
A('hecate_asc_triple_goddess','Deusa Tríplice','△','Combinar um golpe após dash ou uma invocação com outra fonte desperta poder arcano. Ecos e Fogo das Bruxas recebem uma repetição adicional.','ASCENSÃO · MAGIA',{sources:2,duration:4500,damage:.20,synergies:['hecate_triple_path','hecate_arcane_echo','hecate_witchfire']}),
A('hecate_asc_endless_night','Noite Sem Fim','☾','A cada sete acertos, a sombra repete o golpe. Selo da Encruzilhada espalha maldições e Conhecimento Proibido ecoa novamente.','ASCENSÃO · MALDIÇÃO',{hits:7,mult:.65,synergies:['hecate_hex','hecate_forbidden']})
],
selene:[
A('selene_asc_blood_moon','Lua de Sangue','🌕','A lua atinge automaticamente um inimigo a cada cinco segundos. Durante Lua Cheia, ataques causam dano em área e o Raio recarrega mais rápido.','ASCENSÃO · LUA CHEIA',{cooldown:5000,range:300,mult:.65,synergies:['selene_lunar_cycle','selene_moonbeam','selene_moon_harvest']}),
A('selene_asc_eternal_eclipse','Eclipse Eterno','◒','Abaixo de 50% de vida, concede dano e defesa próprios. Com Ciclo Lunar, o bônus da fase anterior permanece; Eclipse desperta mais cedo.','ASCENSÃO · FASES',{threshold:.50,damage:.15,defense:.12,synergies:['selene_lunar_cycle','selene_eclipse']})
],
moros:[
A('moros_asc_sealed_fate','Destino Selado','✴','A cada sete ataques, o destino garante um crítico que causa uma ruptura adicional. Golpe Predestinado também ativa essa ruptura.','ASCENSÃO · DESTINO',{hits:7,critBonus:.50,rupture:1,synergies:['moros_fated_crit','moros_inevitable']}),
A('moros_asc_rewrite_fate','Reescrever o Destino','⌛','Uma vez por onda, reescreve uma morte mesmo sem Destino Adiado. Os três ataques seguintes são críticos e chefes restauram a salvação.','ASCENSÃO · SOBREVIVÊNCIA',{heal:.12,critHits:3,damage:.50,synergies:['moros_delayed_fate']})
],
atena:[
A('atena_asc_absolute_aegis','Égide Absoluta','🛡','Todo dash prepara um contra-ataque próprio. Esquivas perfeitas dão invulnerabilidade; Contra-Ataque e Égide liberam ondas maiores.','ASCENSÃO · DEFESA',{counter:.40,window:2200,invul:500,synergies:['atena_counter_dash','atena_aegis']}),
A('atena_asc_strategos','Estrategista','♜','A cada 10 eliminações sem sofrer dano, ganha uma Medalha que concede dano e defesa e libera uma resposta tática.','ASCENSÃO · TÉCNICA',{kills:10,max:3,damage:.03,defense:.025,synergies:['atena_perfect_stance','atena_tactical_mark']})
],
hermes:[
A('hermes_asc_divine_speed','Velocidade Divina','🪽','Alternar quatro vezes entre ataque e dash ativa Fluxo próprio. No máximo, acertos cortam a recarga do dash e dashes prolongam o estado.','ASCENSÃO · FLUXO',{max:4,duration:4000,damage:.10,attack:.15,move:.12,dashCut:180,synergies:['hermes_flow','hermes_quicksilver']}),
A('hermes_asc_afterimage_army','Exército de Pós-Imagens','»','Cada dash cria duas pós-imagens mesmo sem outra bênção. Pós-Imagem ganha cópias adicionais, mais dano e um segundo alvo.','ASCENSÃO · MOVIMENTO',{charges:2,mult:.30,synergies:['hermes_afterimage']})
],
dionisio:[
A('dionisio_asc_endless_party','Festa Sem Fim','🎉','A cada cinco eliminações recebe um bônus aleatório próprio. Bônus renovam uma vez e completar o trio concede poder geral.','ASCENSÃO · CAOS',{kills:5,power:.10,duration:4000,synergies:['dionisio_party','dionisio_double_toast','dionisio_blackout']}),
A('dionisio_asc_delirium','Delírio Coletivo','🍷','Cinco acertos intoxicam e explodem o alvo. Ressaca espalha cargas para três inimigos e Apagão ganha área e dano.','ASCENSÃO · RESSACA',{hits:5,mult:.55,radius:100,synergies:['dionisio_hangover','dionisio_blackout']})
],
hefesto:[
A('hefesto_asc_living_forge','Forja Viva','◆','Cada fonte de ataque forja seu próprio golpe especial. Obra-Prima vale para todo o arsenal e a arma temperada conserva metade do bônus.','ASCENSÃO · ARSENAL',{hits:7,mult:.55,radius:85,synergies:['hefesto_masterpiece','hefesto_temper']}),
A('hefesto_asc_vulcan_hammer','Martelo de Vulcano','🔨','Acumular dano equivalente a sete golpes básicos faz o Martelo cair em área. Bigorna e Superaquecimento atingem novamente.','ASCENSÃO · IMPACTO',{heat:7,mult:.70,radius:110,synergies:['hefesto_anvil','hefesto_overheat']})
],
artemis:[
A('artemis_asc_wild_hunt','Caçada Selvagem','🏹','Marca automaticamente a primeira Presa e garante um crítico após seis acertos. Ricochetes ganham alvos e a Marca salta após a morte.','ASCENSÃO · CAÇA',{hits:6,synergies:['artemis_mark_prey','artemis_ricochet']}),
A('artemis_asc_moon_arrow','Flecha da Lua','✧','Marca automaticamente uma Presa. A cada quarto acerto nela, uma flecha espectral atravessa vários inimigos.','ASCENSÃO · PRECISÃO',{hits:4,targets:4,mult:.65,synergies:['artemis_mark_prey','artemis_predator','artemis_weakpoint']})
],
poseidon:[
A('poseidon_asc_tsunami','Tsunami','🌊','A cada sete acertos, cria uma onda própria. Ondas de Poseidon ganham área, empurrão e acumulam Pressão mais rápido.','ASCENSÃO · MARÉ',{hits:7,mult:.55,radius:130,push:55,synergies:['poseidon_wave','poseidon_riptide','poseidon_crush','poseidon_surge']}),
A('poseidon_asc_raging_seas','Mares Revoltos','≈','Acertos erguem uma Maré própria. Quando cheia, ela não decai e todo dash libera uma Onda de Impacto.','ASCENSÃO · COMBO',{gain:.015,max:.15,mult:.50,synergies:['poseidon_tide','poseidon_wave']})
],
hercules:[
A('hercules_asc_demigod','Semideus','💪','A cada sete ataques, desfere um golpe titânico próprio. Golpe Titânico ativa mais rápido e Troféu recebe força e duração.','ASCENSÃO · FORÇA',{hits:7,mult:.65,radius:100,synergies:['hercules_slam','hercules_trophy']}),
...
sauron:[
A('sauron_asc_one_ring','Um Anel para Todos','👁','Amplifica todas as bênçãos mesmo sem O Anel, mas sacrifica 15% da vida máxima. O Anel amplia ainda mais o pacto.','ASCENSÃO · CORRUPÇÃO',{hpPenalty:.15,amp:.18,synergies:['sauron_ring']}),
A('sauron_asc_dark_lord','Senhor Sombrio','♛','A cada cinco eliminações, libera uma Nova Sombria e acumula poder. Dominação máxima cria ondas e Corrupção cresce mais rápido.','ASCENSÃO · DOMINAÇÃO',{kills:5,mult:.65,radius:120,growth:.01,cap:.10,synergies:['sauron_domination','sauron_corruption']})
],
nazgul:[
...
A('nazgul_asc_witch_king','Rei Bruxo','🗡','Todo dash prepara um crítico; críticos cortam a recarga do dash. Esquivas também garantem crítico e Golpe Fantasma ganha área.','ASCENSÃO · ASSASSINATO',{dashCut:250,synergies:['nazgul_spectral','nazgul_shadow_dash','nazgul_phantom_blade']})
],
ents:[
A('ents_asc_awakened_forest','Floresta Desperta','🌳','A floresta pulsa, cura e fere inimigos a cada oito segundos. Coração da Floresta acelera e fortalece o pulso.','ASCENSÃO · BOSQUE',{cooldown:8000,heal:.02,mult:.50,radius:140,synergies:['ents_grove','ents_roots']}),
A('ents_asc_last_shepherd','Último Pastor','♚','Cada onda concede crescimento permanente de vida e defesa. Casca cresce em dobro e metade da vida das Sementes é preservada.','ASCENSÃO · CRESCIMENTO',{waveHp:.02,defense:.02,cap:.12,synergies:['ents_bark','ents_seed']})
]
};
