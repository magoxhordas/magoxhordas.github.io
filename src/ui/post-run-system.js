(function(global){
  ...
  function highlights(){
    ...
    if(current.team?.maxCombo)list.push({k:'MAIOR COMBO',v:`x${formatNumber(current.team.maxCombo)}`,d:'sequência eficiente'});
  ...
  function combatTab(){
    ...
    return`<section class="pr-stat-grid compact">${stat('Dano causado',formatNumber(t.damageDealt))}${stat('Dano recebido',formatNumber(t.damageTaken))}${stat('Cura efetiva',formatNumber(t.healing))}${stat('Abates',formatNumber(t.kills))}${stat('Críticos',formatNumber(t.criticals))}${stat('Esquivas',formatNumber(t.dashes))}${stat('Elites',formatNumber(t.elites))}${stat('Minibosses',formatNumber(t.minibosses))}${stat('Chefes',formatNumber(t.bosses))}${stat('Tempo em combate',formatTime(current.combatMs))}${stat('Menor vida',`${formatNumber(lowest)}%`)}${stat('Maior combo',`x${formatNumber(t.maxCombo)}`)}${stat('Maior sequência',formatNumber(t.longestKillStreak))}${stat('Maior multikill',formatNumber(current.maxMultiKill))}${stat('XP recebido',formatNumber(t.xpEarned))}${stat('Moedas obtidas',formatNumber(t.coinsEarned))}${stat('Moedas gastas',formatNumber(t.coinsSpent))}</section><div class="pr-two-col"><section><h2>ORIGEM DO DANO</h2>${sources}</section><section><h2>BATALHAS DE CHEFE</h2>${fights}${current.defeatedBy?`<p class="pr-defeat">DERROTADO POR · ${esc(current.defeatedBy)}</p>`:''}</section></div>`;
  }
  ...
  function debugOpenMock(overrides={}){
    const mock={
      ...
      team:{kills:487,elites:12,minibosses:2,bosses:5,damageDealt:182430,damageTaken:642,healing:125,criticals:100,dashes:55,xpEarned:4380,coinsEarned:412,coinsSpent:214,longestKillStreak:127,maxCombo:143},
