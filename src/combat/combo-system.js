...
(function(global){
  ...
  function notificarAbate(jogador,alvo,origem){
    if(!jogador||!ativoPara(jogador))return 0;
    if(origem==='pet'||origem==='dot'||origem==='environment'||origem==='summon'||origem==='blessing')return 0;
  ...
  function atualizarTier(st){
    ...
    if(st.tierIdx>antes){
      ...
      if(typeof deps.aoMudarTier==='function'){
        deps.aoMudarTier(st.tierIdx,CONFIG.TIERS[st.tierIdx],st.tierIdx>antes,st.jogador);
      }
    }
    if(st.pontos>st.maiorNaOnda)st.maiorNaOnda=st.pontos;
    if(st.pontos>st.maiorNaRun){
      const anterior=Math.floor(st.maiorNaRun);
      st.maiorNaRun=st.pontos;
      const atual=Math.floor(st.maiorNaRun);
      if(atual>anterior&&typeof deps.aoAtualizarRecorde==='function')deps.aoAtualizarRecorde(st.jogador,atual);
    }
  }
  ...
  function maiorComboDaRun(jogador){
    return Math.floor((estados.get(jogador)?.maiorNaRun)||0);
  }
  ...
  global.ComboSystem=Object.freeze({
    ...
    bonusRecompensa,maiorComboDaOnda,maiorComboDaRun,
    reiniciar,limpar,
