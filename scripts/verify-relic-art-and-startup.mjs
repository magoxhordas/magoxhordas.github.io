...
assert.match(html,/artPath:CODEX_RELIC_ART\[spec\.id\]|const artPath=typeof CODEX_RELIC_ART/,'Os itens da campanha não receberam a arte de relíquia.');
assert.match(html,/item\.artPath\s*\?\s*codexArtIconHtml\(item\.artPath,84/,'A loja da campanha não usa os novos ícones.');
assert.match(html,/CODEX_RELIC_ART\[`dng_relic_\$\{item\.id\}`\]/,'O Ferreiro da Dungeon não usa os novos ícones.');
assert.match(html,/window\.DNG_RELICS=DNG_RELICS;/,'As relíquias da Dungeon não estão expostas ao Códex.');
assert.match(html,/window\.DNG_RING_TYPES=DNG_RING_TYPES;/,'Os anéis da Dungeon não estão expostos ao Códex.');
